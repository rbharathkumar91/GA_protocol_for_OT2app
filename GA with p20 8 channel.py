...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # labware
    with open('grenier_96_wellplate_300ul.json') as labware_file:
        labware_def = json.load(labware_file)
        Plasmaplate = protocol.load_labware_from_definition(labware_def, 3)
    
    with open('grenier_96_wellplate_300ul.json') as labware_file:
        labware_def = json.load(labware_file)
        Dilutionplate = protocol.load_labware_from_definition(labware_def, 6)
        
    with open('homemade_4_reservoir_40000ul.json') as labware_file:
        labware_def = json.load(labware_file)
        Reservoir = protocol.load_labware_from_definition(labware_def, 9)
    
    with open('grenier_96_wellplate_300ul.json') as labware_file:
        labware_def = json.load(labware_file)
        Digestionplate = protocol.load_labware_from_definition(labware_def, 7)
        
    with open('grenier_96_wellplate_300ul.json') as labware_file:
        labware_def = json.load(labware_file)
        FAplate = protocol.load_labware_from_definition(labware_def, 8)
        
    #with open('grenier_96_wellplate_300ul.json') as labware_file:
        #labware_def = json.load(labware_file)
        #Trypsinplate = protocol.load_labware_from_definition(labware_def, 5) 
    ...
    for i in range(12):
        right_pipette.transfer (5, Plasmaplate.columns()[i], Dilutionplate.columns()[i], new_tip='always', mix_before= (1,10), mix_after=(2,15),trash=True)
